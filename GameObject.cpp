...
{
    if ((*(_BYTE *)((_QWORD)this + 0x38E8) & 1) != 0)
        return *(const char **)((_QWORD)this + 0x38F8);
    else
        return (const char *)this + 0x38E9;
}
...
bool GameObject::HasUnitTag(std::string tag)
{
    ...
    return std::find(this->Info()->tags.begin(), this->Info()->tags.end(), tag) != this->Info()->tags.end();
}
...
{
    return ((float (*)(__int64, int))(Lol->Address + Offset::LocalPlayer::GetAttackCastDelay))((__int64)this, 0x40);
}

AIManager *GameObject::GetAIManager()
{
    auto a1 = *(_QWORD *)((_QWORD)this + 0x37E0);
...
float Controller::GetPing()
{
    ...
    return ((__int64 (*)(__int64))(Module->Address + Offset::World::GetPing))(*NetClient) / 1000.0f;
}
...
{
    return Me->Level < 9 ? 40.f : 40.f + 4 * (Me->Level - 8);
}

float Controller::OnHitNashorsTooth(GameObject *Me, GameObject *Target)
{
    return 15.f + Me->AbilityPower * 20.f / 100.f;
}
...
Vector3 Controller::PredictPos(GameObject *Target, float PredictTime)
{
    ...
    if (AIBase->SegmentCount <= 1)
        DirectVec = AIBase->EndPos.sub(AIBase->StartPos).normalize();
    else
        DirectVec = AIBase->TargetPaths[1].sub(AIBase->TargetPaths[0]).normalize();
    float Distance = AIBase->MoveSpeed * (PredictTime + GetPing());
    return Target->Position.add(DirectVec.scale(Distance));
}
